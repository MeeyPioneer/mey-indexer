...
type Indexer struct {
	db              db.DbController
	grpcClient      types.MeyCoinRPCServiceClient
	...
	State           string
	BulkState       string
	stream          types.MeyCoinRPCService_ListBlockStreamClient
	startFrom       int64
	stopAt          int64
...
func (ns *Indexer) Start(grpcClient types.MeyCoinRPCServiceClient, reindex bool, exitOnComplete bool, startFrom int64, stopAt int64, idleOnConflict int32) error {
...
func (ns *Indexer) IndexTxs(block *types.Block, txs []*types.Tx, channel chan doc.DocType, nameChannel chan doc.DocType) {
	...
	for _, tx := range txs {
		...
		if tx.GetBody().GetType() == types.TxType_GOVERNANCE && string(tx.GetBody().GetRecipient()) == "meycoin.name" {
