...
import (
	...
	indx "github.com/meeypioneer/mey-indexer/indexer"
...
var (
	rootCmd = &cobra.Command{
		Use:   "indexer",
		Short: "MeyCoin Indexer",
		Long:  "MeyCoin Metadata Indexer",
		Run:   rootRun,
	}
	reindexingMode  bool
	exitOnComplete  bool
	...
	client  types.MeyCoinRPCServiceClient
...
func init() {
	...
	fs.StringVarP(&host, "host", "H", "localhost", "host address of meycoin server")
	fs.Int32VarP(&port, "port", "p", 7845, "port number of meycoin server")
	fs.StringVarP(&meycoinAddress, "meycoin", "A", "", "host and port of meycoin server. Alternative to setting host and port separately.")
...
func getServerAddress() string {
	if len(meycoinAddress) > 0 {
		return meycoinAddress
	}
	return fmt.Sprintf("%s:%d", host, port)
}

func waitForClient(serverAddr string) types.MeyCoinRPCServiceClient {
	...
	for {
		...
		logger.Info().Str("serverAddr", serverAddr).Err(err).Msg("Could not connect to meycoin server, retrying")
		time.Sleep(time.Second)
	}
	logger.Info().Str("serverAddr", serverAddr).Msg("Connected to meycoin server")
	return types.NewMeyCoinRPCServiceClient(conn)
}
